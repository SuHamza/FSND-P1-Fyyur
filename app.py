...
class Venue(db.Model):
    ...
    # One-to-Many Relationship with Show table
...
def venues():
  ...
  temp = [{"city": area.city, "state": area.state, "venues": []} for area in areas]
  for t in temp:
    for venue in venues:
      # Get number of upcoming shows
      show_count = Show.query.filter(Show.venue_id == venue.id, Show.start_time > datetime.now()).count()
      if (venue.city == t['city'] and venue.state == t['state']):
        t['venues'].append({"id": venue.id, "name": venue.name, "num_upcoming_shows": show_count})
  # data=[{
  #   "city": "San Francisco",
  #   "state": "CA",
  #   "venues": [{
  ...
  #   }]
  # }]
  return render_template('pages/venues.html', areas=temp);
...
def search_venues():
  ...
  search_query = Venue.query.filter(Venue.name.ilike('%' + search_term + '%'))
  response={
  ...
  #   }]
  # }
  return render_template('pages/search_venues.html', results=response, q = search_query, search_term=request.form.get('search_term', ''))

@app.route('/venues/<int:venue_id>')
def show_venue(venue_id):
  # shows the venue page with the given venue_id
  # TODO: replace with real venue data from the venues table, using venue_id
  # Search for Venue by ID
  venue = Venue.query.get(venue_id)
  # Search for Venue's Past Shows
  past_shows = Show.query.filter(Show.venue_id == venue.id, Show.start_time < datetime.now())
  # Search for Venue's Upcoming Shows
...
def search_artists():
  ...
  search_term = request.form.get('search_term', '')
  search_query = Artist.query.filter(Artist.name.ilike('%' + search_term + '%'))
  response={
    "count": search_query.count(),
    "data": [{
      "id": result.id,
...
def show_artist(artist_id):
  ...
  # TODO: replace with real venue data from the Artists table, using artist_id
...
def shows():
  ...
  #       num_shows should be aggregated based on number of upcoming shows per venue.
  # Get all available shows in Database
...
def create_show_submission():
  ...
  try:
    ...
    # if(Artist.query.get(artist_id) and Venue.query.get(venue.id)):
    show = Show(artist_id=artist_id, venue_id=venue_id, start_time=start_time)
    db.session.add(show)
    db.session.commit()
    new_show = show
    # else:
    #   flash('Artist or Venue ID Not Found. Show could not be listed!')
  except:
    error = True
    db.session.rollback()
  finally:
